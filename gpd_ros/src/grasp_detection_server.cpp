    ...
    : Node ("detect_grasps_server")
{
    ...
    this->declare_parameter<std::vector<double>> ("workspace", {-1, 1, -1, 1, -1, 1});
    ...
    if (!rviz_topic.empty())
    {
        auto grasps_rviz_pub_ = create_publisher<visualization_msgs::msg::MarkerArray> (rviz_topic, 10);
    ...
    std::shared_ptr<gpd_ros::srv::DetectGrasps::Response> res)
{
    ...
    RCLCPP_INFO_STREAM (
        this->get_logger(),
        "Received cloud with " << cloud_camera_->getCloudProcessed()->size() << " points and " << indices.size()
                               << " samples");
    ...
    if (!grasps.empty())
    {
        ...
        auto grasp_list_msg = GraspMessages::createGraspListMsg (grasps, cloud_camera_header_);
        ...
        RCLCPP_INFO_STREAM (
            this->get_logger(), "Detected " << grasp_list_msg.grasps.size() << " highest-scoring grasps.");
    }
    else
    {
...
int main (int argc, char** argv)
{
    ...
    auto service = node->create_service<gpd_ros::srv::DetectGrasps> (
        "detect_grasps",
        std::bind (&GraspDetectionServer::detectGrasps, node, std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO (node->get_logger(), "Grasp detection service is waiting for a point cloud ...");
