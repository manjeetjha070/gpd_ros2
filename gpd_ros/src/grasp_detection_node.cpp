...
//  }
 ...
 // Read input cloud and sample ROS topics parameters. 
  ...
  this->declare_parameter<int>("cloud_type",POINT_CLOUD_2);
  ...
  this->get_parameter("cloud_type", cloud_type);
  this->get_parameter("cloud_topic", cloud_topic);
  this->get_parameter("samples_topic", samples_topic);
  ...
  {
    grasps_rviz_pub_ =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(rviz_topic,10);
    use_rviz_ = true;
  }
  else
  {
  ...
  {
    cloud_sub_pc2_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      cloud_topic,
      10,
      std::bind(&GraspDetectionNode::cloud_callback,this,std::placeholders::_1));
  }
  else if (cloud_type == CLOUD_INDEXED)
  {
    cloud_sub_cloud_indexed_ = this->create_subscription<gpd_ros::msg::CloudIndexed>(
      cloud_topic,
      10,
      std::bind(&GraspDetectionNode::cloud_indexed_callback,this,std::placeholders::_1));
  }
  else if (cloud_type == CLOUD_SAMPLES)
  {
    cloud_sub_cloud_samples_ = this->create_subscription<gpd_ros::msg::CloudSamples>(
      cloud_topic,
      10,
      std::bind(&GraspDetectionNode::cloud_samples_callback,this,std::placeholders::_1));
  ...
  {
    samples_sub_ = this->create_subscription<gpd_ros::msg::SamplesMsg>(
      samples_topic,
      10,
      std::bind(&GraspDetectionNode::samples_callback,this,std::placeholders::_1));
  ...
  grasps_pub_ =
    this->create_publisher<gpd_ros::msg::GraspConfigList>("clustered_grasps",10);

  rviz_plotter_ =
    new GraspPlotter(this->shared_from_this(),
    grasp_detector_->getHandSearchParameters().hand_geometry_);

  RCLCPP_INFO(this->get_logger(),"GPD initialized");
}
...
void GraspDetectionNode::run()
{
  ...
  RCLCPP_INFO(this->get_logger(),"Waiting for point cloud...");
...
std::vector<std::unique_ptr<gpd::candidate::Hand>>
GraspDetectionNode::detectGraspPoses()
{
    // detect grasp poses
  ...
  if(use_importance_sampling_)
  {
    ...
    RCLCPP_ERROR(this->get_logger(),"Importance sampling not supported");
  }
  else
  {
  ...
  auto msg =
    GraspMessages::createGraspListMsg(grasps, cloud_camera_header_);

  grasps_pub_->publish(msg);

  RCLCPP_INFO(this->get_logger(),
      "Published %ld grasps", msg.grasps.size());
...
void GraspDetectionNode::cloud_callback(
  const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  if(msg->fields.size()==6 &&
     msg->fields[3].name=="normal_x" && msg.fields[4].name == "normal_y"
      && msg.fields[5].name == "normal_z")
  {
    ...
    cloud_camera_ =
      new gpd::util::Cloud(cloud,0,view_points);

    RCLCPP_INFO(this->get_logger(),
      "Cloud with normals: %ld points",
      cloud->size());
  }
  else
  {
...
void GraspDetectionNode::cloud_indexed_callback(
...
void GraspDetectionNode::cloud_samples_callback(
  const gpd_ros::msg::CloudSamples::SharedPtr msg)
{
  if (!has_cloud_)
  {
    ...
    RCLCPP_INFO_STREAM(
      ...
      << cloud_camera_->getCloudProcessed()->size()
      << " points and "
      << cloud_camera_->getSamples().cols()
      << " samples");
  }
}

void GraspDetectionNode::samples_callback(
  const gpd_ros::msg::SamplesMsg::SharedPtr msg)
{
  if (!has_samples_)
  {
    ...
    RCLCPP_INFO_STREAM(
      this->get_logger(),
      "Received grasp samples message with "
      << msg->samples.size()
      << " samples");
  }
}

void GraspDetectionNode::initCloudCamera(
  const gpd_ros::msg::CloudSources& msg)
{
  ...
  {
    view_points.col(i) << msg.view_points[i].x,
                          msg.view_points[i].y,
                          msg.view_points[i].z;
  }

  // Set point cloud.
  if (msg.cloud.fields.size() == 6 &&
